...
function CitationRow({
  ...
  const className =
    "group/citation flex items-center gap-2 rounded-md px-1.5 py-1 outline-none focus-visible:ring-2 focus-visible:ring-ring w-full text-left";
  ...
  return (
    <div className="flex flex-col w-full">
      {citation.url ? (
        ...
        <button id={id} className={className} onClick={() => setOpen(!open)} type="button">
          {content}
        </button>
      ) : (
      ...
      {citation.content && (
        <AgentDisclosure open={open}>
          <div className="pl-[2.125rem] pr-2 pb-2 pt-1 text-xs text-muted-foreground/80 whitespace-pre-wrap font-mono break-all">
            {citation.content}
          </div>
        </AgentDisclosure>
      )}
